...
class PrecomputeProcessor:
    ...
    def __init__(self, db_path: str, election_id: str = "2024_portland_district2"):
        ...
        self.data_dir = Path(__file__).parent.parent / "data" / "elections" / election_id
    ...
    def validate_prerequisites(self) -> bool:
        ...
        if missing_tables:
            ...
            logger.error("Please run 'python scripts/process_data.py' first to load basic data")
            return False
        
        # Check data quality
        ballot_count = self.db.query("SELECT COUNT(DISTINCT BallotID) as count FROM ballots_long")['count'].iloc[0]
        candidate_count = self.db.query("SELECT COUNT(*) as count FROM candidates")['count'].iloc[0]
    ...
    def precompute_adjacent_pairs(self, min_shared_ballots: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Computing pairwise relationships (min {min_shared_ballots} shared ballots)...")
            ...
            logger.info(f"✓ Precomputed {stats['total_pairs']} candidate pairs in {operation_time:.2f}s")
            ...
            self.stats['performance_improvements']['adjacent_pairs'] = {
                ...
                'api_endpoints_affected': ['/api/coalition/pairs/all', '/api/coalition/network']
            }
            self.stats['data_sizes']['adjacent_pairs_mb'] = parquet_path.stat().st_size / (1024 * 1024)
    ...
    def precompute_candidate_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            operation_time = time.time() - operation_start
            logger.info(f"✓ Precomputed metrics for {metrics_stats['total_candidates']} candidates in {operation_time:.2f}s")
            logger.info(f"  - Central hubs: {metrics_stats['central_hubs']}")
            logger.info(f"  - Well connected: {metrics_stats['well_connected']}")
            logger.info(f"  - Moderately connected: {metrics_stats['moderately_connected']}")
            ...
            self.stats['performance_improvements']['candidate_metrics'] = {
                ...
                'api_endpoints_affected': ['/api/candidates/enhanced', '/api/coalition/network']
            }
            self.stats['data_sizes']['candidate_metrics_mb'] = parquet_path.stat().st_size / (1024 * 1024)
    ...
    def precompute_static_responses(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.stats['performance_improvements']['static_responses'] = {
                ...
                'api_endpoints_affected': ['/api/coalition/types', '/api/candidates/enhanced']
            }
    ...
    def generate_metadata(self) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            'data_quality': {
                'total_pairs': self.db.query("SELECT COUNT(*) as count FROM adjacent_pairs")['count'].iloc[0],
                'total_candidates': self.db.query("SELECT COUNT(*) as count FROM candidate_metrics")['count'].iloc[0],
                'avg_coalition_strength': float(self.db.query("SELECT AVG(coalition_strength_score) as avg FROM adjacent_pairs")['avg'].iloc[0])
            }
        }
...
def main():
    parser = argparse.ArgumentParser(description="Precompute analytical data for performance optimization")
    parser.add_argument("--db", required=True, help="Path to DuckDB database file")
    parser.add_argument("--election-id", default="2024_portland_district2", help="Election identifier")
    parser.add_argument("--min-shared-ballots", type=int, default=10, help="Minimum shared ballots for pair analysis")
    parser.add_argument("--force-refresh", action="store_true", help="Force rebuild of all precomputed data")
    parser.add_argument("--validate", action="store_true", help="Run validation checks after precomputation")
    ...
    try:
        ...
        if not args.force_refresh and (processor.precomputed_dir / "adjacent_pairs.parquet").exists():
            logger.info("Precomputed data already exists. Use --force-refresh to rebuild.")
            ...
            if metadata_path.exists():
                ...
                logger.info(f"Existing data version: {metadata.get('data_version', 'unknown')}")
        ...
        if args.validate:
            ...
            pairs_count = db.query("SELECT COUNT(*) as count FROM adjacent_pairs")['count'].iloc[0]
            logger.info(f"✓ Adjacent pairs table: {pairs_count:,} records")
            
            # Check candidate metrics
            metrics_count = db.query("SELECT COUNT(*) as count FROM candidate_metrics")['count'].iloc[0]
            ...
            logger.info(f"✓ Coalition strength range: {strength_stats['min_strength']:.4f} - {strength_stats['max_strength']:.4f}")
            logger.info(f"✓ Average coalition strength: {strength_stats['avg_strength']:.4f}")
            
            # Check for winners in candidate metrics
            winners = db.query("SELECT candidate_name FROM candidate_metrics WHERE candidate_id IN (36, 46, 55) ORDER BY weighted_score DESC")
            logger.info(f"✓ Portland winners found: {', '.join(winners['candidate_name'])}")
