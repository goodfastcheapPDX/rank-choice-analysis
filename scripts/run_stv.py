...
def main():
    ...
    if not args.db or not Path(args.db).exists():
        logger.error("Database file required and must exist. Run process_data.py first.")
        sys.exit(1)
        ...
        with CVRDatabase(args.db) as db:
            ...
            for table in required_tables:
                if not db.table_exists(table):
                    logger.error(f"Required table '{table}' not found. Run process_data.py first.")
                    sys.exit(1)
            ...
            for round_num in sorted(round_summary['round'].unique()):
                ...
                for _, row in round_data.sort_values('votes', ascending=False).iterrows():
                    candidate_name = candidate_names.get(row['candidate_id'], f"ID-{row['candidate_id']}")
                    status_symbol = {
                        'elected': '🏆',
                        'eliminated': '❌',
                        'continuing': '  ',
                    ...
                    print(f"  {status_symbol} {candidate_name:25s}: {row['votes']:8.1f} votes")
                
                if round_data.iloc[0]['exhausted_votes'] > 0:
                    print(f"     {'Exhausted':25s}: {round_data.iloc[0]['exhausted_votes']:8.1f} votes")
            ...
            for i, (_, row) in enumerate(elected.iterrows(), 1):
                print(f"  {i}. {row['candidate_name']:30s}: {row['final_votes']:8.1f} votes (Round {row['election_round']})")
            
            print(f"\nNot Elected:")
            not_elected = final_results[final_results['status'] == 'not_elected'].head(10)
            for _, row in not_elected.iterrows():
                print(f"     {row['candidate_name']:30s}: {row['final_votes']:8.1f} votes")
            ...
            if args.export:
                ...
                round_summary_export.to_csv(
                    export_path.with_stem(export_path.stem + '_rounds').with_suffix('.csv'), 
                    index=False
                )
                print(f"✓ Round summary exported to: {export_path.with_stem(export_path.stem + '_rounds').with_suffix('.csv')}")
