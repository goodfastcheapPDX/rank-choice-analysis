...
def test_with_sample_data():
    ...
    if not cvr_files:
        ...
        print(
            "Please copy your CVR data file (e.g., 'City_of_Portland__Councilor__District_2_2024_11_29_17_26_12.cvr copy.csv') to the project root for testing."
        )
        return False
    ...
    try:
        ...
        with CVRParser(db_path) as parser:
            ...
            print(
                f"✓ Top candidate: {first_choice.iloc[0]['candidate_name']} with {first_choice.iloc[0]['first_choice_votes']} votes"
            )
        ...
        if official_results.exists():
            ...
            with CVRParser(db_path) as verification_parser:
                ...
                candidates = verification_parser.db.query(
                    "SELECT candidate_id, candidate_name FROM candidates"
                )
                first_choice = verification_parser.db.query(
                    "SELECT * FROM first_choice_totals"
                )
                ...
                if verification_results["verification_passed"]:
                    ...
                    print(
                        f"   Vote differences: {verification_results['total_vote_difference']}"
                    )
        else:
        ...
        print(
            f"python scripts/verify_results.py --db {db_path} --official {official_results}"
        )
