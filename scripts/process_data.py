...
def main():
    ...
    parser.add_argument("--db", help="Path to DuckDB database file (default: in-memory)")
    ...
    try:
        with CVRParser(args.db) as parser:
            ...
            if load_stats['duplicate_ballots'] > 0:
                print(f"⚠️  Warning: {load_stats['duplicate_ballots']} duplicate ballot IDs")
            ...
            for _, candidate in candidates.iterrows():
                print(f"  {candidate['candidate_id']:2d}: {candidate['candidate_name']}")
            ...
            for _, row in first_choice.head(10).iterrows():
                print(f"  {row['candidate_name']:25s}: {row['first_choice_votes']:5d} votes ({row['percentage']:5.1f}%)")
            ...
            for _, row in completion.iterrows():
                print(f"  {row['ranks_used']} ranks: {row['ballot_count']:5d} ballots ({row['percentage']:5.1f}%)")
            ...
            if args.validate:
                ...
                rank1_votes = votes_by_rank[votes_by_rank['rank_position'] == 1]['total_votes'].sum()
                rank1_rate = rank1_votes / total_ballots
                
                print(f"\nValidation Results:")
                print(f"✓ Rank 1 participation: {rank1_rate:.1%} ({rank1_votes}/{total_ballots})")
