...
def test_implementations(db_path: str):
    ...
    if not Path(db_path).exists():
        ...
        logger.info("Please run: python scripts/process_data.py your_election_file.csv --db election_data.db")
        return
    ...
    try:
        ...
        logger.info(f"Database contains {candidates.iloc[0]['count']} candidates and {ballots.iloc[0]['count']} ballots")
        ...
        if original_winners == pyrankvote_winners:
            ...
            logger.warning(f"  Only in original: {sorted(original_winners - pyrankvote_winners)}")
            logger.warning(f"  Only in PyRankVote: {sorted(pyrankvote_winners - original_winners)}")
        ...
        if original_quota == pyrankvote_quota:
            ...
            logger.warning(f"❌ Quota differs: Original={original_quota}, PyRankVote={pyrankvote_quota}")
        
        # Show detailed results if different
        if original_winners != pyrankvote_winners:
            logger.info("\n=== Detailed Original Results ===")
            print(original_results[['candidate_id', 'candidate_name', 'final_votes', 'status']].head(10))
            
            logger.info("\n=== Detailed PyRankVote Results ===")
            print(pyrankvote_results[['candidate_id', 'candidate_name', 'final_votes', 'status']].head(10))
