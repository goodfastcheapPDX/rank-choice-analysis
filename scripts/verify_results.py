...
def main():
    parser = argparse.ArgumentParser(
        description="Verify STV results against official results"
    )
    parser.add_argument(
        "--db", required=True, help="Path to DuckDB database file with processed data"
    )
    parser.add_argument(
        "--official", required=True, help="Path to official results CSV file"
    )
    parser.add_argument(
    ...
    try:
        with CVRDatabase(args.db) as db:
            ...
            logger.info(
                f"Official threshold: {official_data['metadata'].get('threshold', 'Unknown')}"
            )
            ...
            if args.export:
                ...
                vote_comparison_path = export_path.with_stem(
                    export_path.stem + "_vote_comparison"
                ).with_suffix(".csv")
                verification_results["vote_comparisons"].to_csv(
                    vote_comparison_path, index=False
                )
