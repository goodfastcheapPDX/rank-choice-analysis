...
class PyRankVoteSTVTabulator:
    ...
    def _prepare_pyrankvote_data(self):
        ...
        candidates_df = self.db.query("SELECT candidate_id, candidate_name FROM candidates ORDER BY candidate_id")
        ...
        for ballot_id, group in grouped_ballots:
            ...
            for _, row in group.sort_values('rank_position').iterrows():
                ...
                if candidate_id in self.candidates_map and candidate_id not in seen_candidates:
    ...
    def run_stv_tabulation(self) -> List[STVRound]:
        ...
        logger.info(f"Running STV election with {len(self.candidates_map)} candidates, {len(self.ballots_data)} ballots, {self.seats} seats")
        
        try:
            # Check if we have more seats than candidates
            if self.seats >= len(self.candidates_map):
                logger.warning(f"Seats ({self.seats}) >= candidates ({len(self.candidates_map)}), electing all candidates")
