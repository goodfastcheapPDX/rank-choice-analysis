...
class CandidateAffinity:
    ...
    overlap_percentage: float  # Percentage of candidate_1's supporters who also support candidate_2
...
class CoalitionAnalyzer:
    ...
    def calculate_pairwise_affinity(self, min_shared_ballots: int = 100) -> List[CandidateAffinity]:
        """
        ...
        ballot_lookup = dict(zip(self.ballot_counts['candidate_id'], self.ballot_counts['total_ballots']))
        ...
        for _, row in cooccur_df.iterrows():
            ...
            if total_1 > 0 and total_2 > 0:
                ...
                affinities.append(CandidateAffinity(
    ...
    def calculate_detailed_pairwise_analysis(self, min_shared_ballots: int = 10) -> List[DetailedCandidatePair]:
        ...
        pair_groups = proximity_df.groupby(['candidate_1', 'candidate_2', 'name_1', 'name_2'])
        ...
        for (cand1_id, cand2_id, name1, name2), group in pair_groups:
            ...
            # Proximity-weighted metrics
            strong_votes = group[group['ranking_distance'] <= 2]['occurrence_count'].sum()
        ...
        logger.info(f"Calculated detailed analysis for {len(detailed_pairs)} candidate pairs")
        return detailed_pairs
    
    def _classify_coalition_type(self, avg_distance: float, strong_votes: int, weak_votes: int, total_votes: int) -> str:
    ...
    def find_vote_transfer_patterns(self, from_candidate: int) -> Dict[int, Dict]:
        ...
        transfers = {}
        total_transfers = transfers_df['transfer_votes'].sum() if not transfers_df.empty else 0
        ...
        for _, row in transfers_df.iterrows():
            transfers[row['next_candidate']] = {
                ...
                'transfer_percentage': (row['transfer_votes'] / total_transfers * 100) if total_transfers > 0 else 0
            }
        
        logger.info(f"Found transfer patterns for {len(transfers)} candidates")
        return transfers
    
    def identify_coalitions(self, min_coalition_size: int = 2, min_support: int = 500) -> List[CoalitionGroup]:
        """
        ...
        for i, affinity in enumerate(affinities[:10]):  # Top 10 affinities
            if affinity.shared_ballots >= min_support:
                coalition = CoalitionGroup(
                    ...
                    candidate_names=[affinity.candidate_1_name, affinity.candidate_2_name],
    ...
    def get_candidate_coalition_summary(self, candidate_id: int) -> Dict:
        ...
        # Get ballot count
        ballot_count = self.ballot_counts[self.ballot_counts['candidate_id'] == candidate_id]
        total_ballots = ballot_count.iloc[0]['total_ballots'] if not ballot_count.empty else 0
    ...
    def analyze_ranking_proximity(self, candidate_1: int, candidate_2: int) -> Dict:
        ...
        """
        logger.info(f"Analyzing ranking proximity for candidates {candidate_1} and {candidate_2}")
        ...
        if proximity_df.empty:
            return {"error": f"No shared ballots found for candidates {candidate_1} and {candidate_2}"}
        ...
        distance_counts = {int(k): int(v) for k, v in proximity_df.groupby('ranking_distance')['occurrence_count'].sum().to_dict().items()}
        
        # Get candidate names
        self._load_candidate_data()
        names = dict(zip(self.candidates_df['candidate_id'], self.candidates_df['candidate_name']))
        ...
        result = {
            ...
            "close_rankings": sum(count for dist, count in distance_counts.items() if dist <= 2),
            "distant_rankings": sum(count for dist, count in distance_counts.items() if dist >= 4)
        }
        ...
        for pair in detailed_pairs:
            ...
            if len(type_examples[coalition_type]) < 3:
                type_examples[coalition_type].append({
                    ...
                    "coalition_strength": float(round(pair.coalition_strength_score, 3))
                })
    ...
    def get_detailed_pair_analysis(self, candidate_1: int, candidate_2: int) -> Optional[DetailedCandidatePair]:
        """
