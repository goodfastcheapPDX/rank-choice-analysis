...
class DatabaseConnectionManager:
    ...
    def get_connection(self, db_path: str, read_only: bool = True, max_retries: int = 3) -> duckdb.DuckDBPyConnection:
        """
        ...
        raise Exception(f"Could not establish database connection after {max_retries} attempts")
...
class CVRDatabase:
    ...
    def conn(self) -> duckdb.DuckDBPyConnection:
        ...
        if self._conn is None:
            self._conn = _connection_manager.get_connection(self.db_path, self.read_only)
        return self._conn
    ...
    def query(self, sql: str, use_temporary_connection: bool = False) -> pd.DataFrame:
        ...
        if use_temporary_connection:
            with _connection_manager.get_temporary_connection(self.db_path, read_only=True) as temp_conn:
    ...
    def table_exists(self, table_name: str, use_temporary_connection: bool = True) -> bool:
        """
