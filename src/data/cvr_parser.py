...
class CVRParser:
    ...
    def _is_ballots_long_current(self) -> bool:
        ...
        try:
            ...
            ballots_long_count = self.db.query(
            ...
            if ballots_long_count < 1000:
                logger.warning(
                    f"ballots_long appears to have insufficient data: {ballots_long_count:,} ballots"
                )
                return False
            ...
            logger.info(
                f"✓ ballots_long is current: {ballots_long_count:,} ballots normalized"
            )
            return True
    ...
    def analyze_candidate_partners(self, candidate_name: str) -> pd.DataFrame:
        ...
        return self.db.query(
            f"SELECT * FROM analyze_candidate_partners('{candidate_name}')"
        )
    ...
    def search_ballots(
        self, candidate_name: str, rank_position: int = 1, limit: int = 10
    ) -> pd.DataFrame:
        """
    ...
    def get_candidates(self) -> pd.DataFrame:
        ...
        if self._candidates is None:
            self._candidates = self.db.query(
